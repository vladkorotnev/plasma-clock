...
const melody_sequence_t * melody_from_no(int melody_no) {
    ...
    no  = std::max(no, -1);
    if(no == all_chime_count || no == -1) {
        static const int boring_melody_no = 4;
        static int last_random_no = -1;
        do {
            no = boring_melody_no + (esp_random() % (all_chime_count - boring_melody_no));
...
#include "melodies/scar_tissue.h"
#include "melodies/space_harrier.h"
